...
export class LocalEnvironmentRepository implements IRepository {
   constructor(private readonly folder: string = 'environments') {}
   ...
   public async exists(): Promise<boolean> {
      return fs.existsSync(path.join(process.cwd(), 'environments'))
   }
   ...
   public async getKey(
      ...
      const variables = environment.split('\n').map((line) => line.trim())
   ...
   public async load({
      example,
      environments
   }: WenviExportableContent): Promise<void> {
      if (!(await this.exists())) {
         await this.init()
      }
      ...
      for (const environment of environments) {
         const isCreated = await this.isEnvironmentCreated(
            environment.subject,
   ...
   public async isEnvironmentCreated(
      subject: string,
      name: string
   ): Promise<boolean> {
