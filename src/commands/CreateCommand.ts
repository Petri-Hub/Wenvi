...
export class CreateCommand extends BaseCommand implements IExecutableCommand {
   public async execute({
      parameters: [subjectName, environmentName, environment]
   ...
   public async createSubject(subjectName: string): Promise<void> {
      ...
      Logger.success(
         `Subject ${chalk.bold.underline(subjectName)} created successfully.`
      )
   }
   ...
   public async createEnvironment(
      ...
      Logger.success(
         `Environment ${chalk.bold.underline(environmentName)} created successfully for subject ${chalk.bold.underline(subjectName)}.`
      )
   }
}
