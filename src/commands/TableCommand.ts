...
export class TableCommand extends BaseCommand implements IExecutableCommand {
   ...
   private async createTableRows(
      parameters: string[]
   ): Promise<HorizontalTableRow[]> {
      const rows = []
      ...
      for (const subjectName of subjects) {
         const row: HorizontalTableRow = [
            chalk
               .hex(TextToColorConverter.convert(subjectName))
               .bold(subjectName)
         ]

         for (const environmentName of environments) {
            const hasEnvironment =
               await this.getRepository().isEnvironmentCreated(
                  subjectName,
                  environmentName
               )

            hasEnvironment ? row.push('✅') : row.push('❌')
         }
   ...
   private async getAllPossibleEnvironmentNames(): Promise<string[]> {
      ...
      const sortedEnvironemnts = Array.from(uniqueEnvironments).sort(
