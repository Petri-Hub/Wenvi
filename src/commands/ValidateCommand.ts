...
export class ValidateCommand extends BaseCommand implements IExecutableCommand {
   ...
   private async validateEnvironment(
      ...
      const missingKeys = example.filter(
         (exampleVariable) => !variables.includes(exampleVariable)
      )

      const additionalKeys = variables.filter(
         (variable) => !example.includes(variable)
      )
