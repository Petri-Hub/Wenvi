...
export class DeleteCommand extends BaseCommand implements IExecutableCommand {
   ...
   private async deleteSubject(subjectName: string): Promise<void> {
      ...
      Logger.success(
         `Subject ${chalk.bold.underline(subjectName)} deleted successfully.`
      )
   }
   ...
   private async deleteEnvironment(
      ...
      Logger.success(
         `Environment ${chalk.bold.underline(environmentName)} deleted successfully for subject ${chalk.bold.underline(subjectName)}.`
      )
   }
}
