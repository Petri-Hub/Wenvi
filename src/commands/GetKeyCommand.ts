...
export class GetKeyCommand extends BaseCommand implements IExecutableCommand {
   public async execute({
      parameters: [subjectName, environmentName, variableKey]
   }: CommandInput) {
      if (!subjectName) {
      ...
      const value = await this.getRepository().getKey(
         subjectName,
         environmentName,
         variableKey
      )
      ...
      Logger.log(
         `Value of ${chalk.underline.bold(variableKey)} in ${chalk.underline.bold(subjectName)} > ${chalk.underline.bold(environmentName)} is: ${value}`
      )
   }
}
